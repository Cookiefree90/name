...
def get_fast_api_app(
  ...
  if web:
    ...
    async def auth_google_callback(
      ...
      try:
        handler = AuthHandler(auth_config)
        temp_state_for_auth: dict[str, Any] = {}
        await handler.parse_and_store_auth_response(state=temp_state_for_auth)
        stored_credential = handler.get_auth_response(state=temp_state_for_auth)
        if (
